...
export class PostsService {
  ...
  // Lấy bài viết của 1 tác giả
  async findByAuthor(authorId: string): Promise<Post[]> {
    return this.postModel.find({ author: authorId, isDeleted: false }).exec();
  }
  ...
  // Cập nhật bài viết (chỉ content và caption)
  async update(
    id: string,
    updatePostDto: Partial<Pick<Post, 'content' | 'caption'>>,
  ): Promise<Post | null> {
    const allowed: Partial<Post> = {};
    if (typeof updatePostDto.content === 'string')
      allowed.content = updatePostDto.content;
    if (
      typeof updatePostDto.caption === 'string' ||
      updatePostDto.caption === undefined
    )
      allowed.caption = updatePostDto.caption;

    return this.postModel.findByIdAndUpdate(id, allowed, { new: true }).exec();
  }
